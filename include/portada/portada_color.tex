...
\begin{titlepage}
	...
	\pagecolor{\colorAzulFIUM}
	\begin{tikzpicture}[overlay, remember picture, inner sep=0pt, outer sep=0pt]
		\node[anchor=north west,inner sep=0pt] at ($(current page.north west)+(1.1cm,-0.3cm)$)
			{\includegraphics[width=6.5cm]{\logoUniversidadPortada}};
	\end{tikzpicture}
	
	% Logo de la facultad en la esquina superior derecha
	%\begin{tikzpicture}[remember picture,overlay]
	%	\node[anchor=north west,inner sep=0pt] at ($(current page.north west)+(15.65cm,-1.3cm)$)
	...
	\node[anchor=south west,inner sep=0pt] at ($(current page.south west)+(12cm,1.6cm)$)
	              {\includegraphics[width=8.2cm]{\logoFacultadPortadaBaja}};
	\end{tikzpicture}
	...
	\begin{textblock*}{\textwidth}(0.3cm,-2.35cm)% Ancho - Pos X,PosY
		...
		{\FuentePortada \fontsize{\TamOtros}{30pt}\selectfont\color{white}\Hoy \hspace{1em}Convocatoria de \Convocatoria}
	\end{textblock*}
